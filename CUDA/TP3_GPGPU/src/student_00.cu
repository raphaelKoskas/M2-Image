...
namespace IMAC
{
    ...
    void maxReduce_ex1(const uint *const dev_array, const uint size, uint *const dev_partialMax)
	{
		...
		if(idx<size)sharedMemory[threadIdx.x] = dev_array[idx];
		__syncthreads();

		while (i<size/2){
			if (i*threadIdx.x < size-1)
				if (sharedMemory[i*threadIdx.x]<sharedMemory[i*threadIdx.x+1])sharedMemory[i*threadIdx.x]=sharedMemory[i*threadIdx.x+1];
			i*=2;
			__syncthreads();
		}
		if (idx==0){
			dev_partialMax[0]=sharedMemory[0];
			printf("test %d\n", dev_partialMax[0]);
		}
	}
