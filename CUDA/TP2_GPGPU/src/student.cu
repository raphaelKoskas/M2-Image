...
namespace IMAC
{
	...
	__global__ void convCUDA(	const uchar4* const input, const uint imgWidth, const uint imgHeight,
								const float* const matConv, const uint matSize, 
								uchar4* output){
		int idx = blockDim.x * blockIdx.x + threadIdx.x,
			idy = blockDim.y * blockIdx.y + threadIdx.y,
			dX,dY, idOut = idy * imgWidth + idx, idMat, idPixel;
	...
	__global__ void convConstMemCUDA(	const uchar4* const input, const uint imgWidth, const uint imgHeight,
	...
	__global__ void conv1DCUDA(	const uint imgWidth, const uint imgHeight,const uint matSize, uchar4* output){
		int idx = blockDim.x * blockIdx.x + threadIdx.x,
			idy = blockDim.y * blockIdx.y + threadIdx.y;
		if(idx >= imgWidth || idy >= imgHeight)
		{
		...
		for (int j = 0; j < matSize; j++){
			...
			for (int i = 0; i < matSize; i++){
				...
				idMat		= j * matSize + i;
				idPixel	= dY * imgWidth + dX;
				pixel = tex1Dfetch(dev_img_1D, idPixel);
				sum.x += (float)pixel.x * gdev_matConv[idMat];
				sum.y += (float)pixel.y * gdev_matConv[idMat];
				sum.z += (float)pixel.z * gdev_matConv[idMat];
			}
...
// ==================================================
					...
					)
	{
		...
		chrGPU.start();
		cudaMalloc((void **) &dev_inputImg,bytes);
		cudaMalloc((void **) &dev_matConv,matrix_bytes);
		cudaMalloc((void **) &dev_output,bytes);
		chrGPU.stop();
		std::cout 	<< "-> Naive Version (Allocation time) : " << chrGPU.elapsedTime() << " ms" << std::endl << std::endl;

		chrGPU.start();
		cudaMemcpy(dev_inputImg,inputImg.data(),bytes,cudaMemcpyHostToDevice);
		cudaMemcpy(dev_matConv,matConv.data(),matrix_bytes,cudaMemcpyHostToDevice);
		chrGPU.stop();
		std::cout 	<< "-> Naive Version (Data Transfer to GPU time) : " << chrGPU.elapsedTime() << " ms" << std::endl << std::endl;
		...
		chrGPU.start();
		convCUDA<<<dim3(nbBlocksX,nbBlocksY),dim3(nbThreadsX,nbThreadsY)>>>(dev_inputImg,imgWidth,imgHeight,dev_matConv,matSize,dev_output);
		chrGPU.stop();
		std::cout 	<< "-> Naive Version (Processing on GPU time) : " << chrGPU.elapsedTime() << " ms" << std::endl << std::endl;

		chrGPU.start();
		cudaMemcpy(output.data(),dev_output,bytes,cudaMemcpyDeviceToHost);
		chrGPU.stop(); 
		std::cout 	<< "-> Naive Version (Data Transfer from GPU time) : " << chrGPU.elapsedTime() << " ms" << std::endl << std::endl;
		std::cout 	<< "-> Naive Version (Data Transfer total time) : " << chrGPU.elapsedTime()+elapsedTime << " ms" << std::endl << std::endl;
		...
		cudaMalloc((void **) &dev_output,bytes);
		chrGPU.stop();
		std::cout 	<< "-> Constant Memory Version (Allocation time) : " << chrGPU.elapsedTime() << " ms" << std::endl << std::endl;

		chrGPU.start();
		cudaMemcpy(dev_inputImg,inputImg.data(),bytes,cudaMemcpyHostToDevice);
		cudaMemcpyToSymbol(gdev_matConv,matConv.data(),matrix_bytes,0,cudaMemcpyHostToDevice);
		chrGPU.stop();
		std::cout 	<< "-> Constant Memory Version (Data Transfer to GPU time) : " << chrGPU.elapsedTime() << " ms" << std::endl << std::endl;
		elapsedTime=chrGPU.elapsedTime();
		...
		chrGPU.start();
		convConstMemCUDA<<<dim3(nbBlocksX,nbBlocksY),dim3(nbThreadsX,nbThreadsY)>>>(dev_inputImg,imgWidth,imgHeight,matSize,dev_output);
		chrGPU.stop();
		std::cout 	<< "-> Constant Memory Version (Processing on GPU time) : " << chrGPU.elapsedTime() << " ms" << std::endl << std::endl;

		chrGPU.start();
		cudaMemcpy(output.data(),dev_output,bytes,cudaMemcpyDeviceToHost);
		chrGPU.stop(); 
		std::cout 	<< "-> Constant Memory Version (Data Transfer from GPU time) : " << chrGPU.elapsedTime() << " ms" << std::endl << std::endl;
		std::cout 	<< "-> Constant Memory Version (Data Transfer total time) : " << chrGPU.elapsedTime()+elapsedTime << " ms" << std::endl << std::endl;
		compareImages(resultCPU, output);
		...
		cudaMalloc((void **) &dev_output,bytes);
		chrGPU.stop();
		std::cout 	<< "-> 1D Texture Version (Allocation time) : " << chrGPU.elapsedTime() << " ms" << std::endl << std::endl;

		chrGPU.start();
		cudaMemcpy(dev_inputImg,inputImg.data(),bytes,cudaMemcpyHostToDevice);
		cudaMemcpyToSymbol(gdev_matConv,matConv.data(),matrix_bytes,0,cudaMemcpyHostToDevice);
		cudaBindTexture( 0, dev_img_1D, dev_inputImg, bytes);
		chrGPU.stop();
		std::cout 	<< "-> 1D Texture Version (Data Transfer to GPU time) : " << chrGPU.elapsedTime() << " ms" << std::endl << std::endl;
		elapsedTime=chrGPU.elapsedTime();
		...
		chrGPU.start();
		conv1DCUDA<<<dim3(nbBlocksX,nbBlocksY),dim3(nbThreadsX,nbThreadsY)>>>(imgWidth,imgHeight,matSize,dev_output);
		chrGPU.stop();
		std::cout 	<< "-> 1D Texture Version (Processing on GPU time) : " << chrGPU.elapsedTime() << " ms" << std::endl << std::endl;

		chrGPU.start();
		cudaMemcpy(output.data(),dev_output,bytes,cudaMemcpyDeviceToHost);
		chrGPU.stop(); 
		std::cout 	<< "-> 1D Texture Version (Data Transfer from GPU time) : " << chrGPU.elapsedTime() << " ms" << std::endl << std::endl;
		std::cout 	<< "-> 1D Texture Version (Data Transfer total time) : " << chrGPU.elapsedTime()+elapsedTime << " ms" << std::endl << std::endl;
		compareImages(resultCPU, output);
		...
		cudaFree(dev_inputImg);cudaFree(dev_output);cudaFree(dev_matConv);
